#!/usr/bin/env bash
# Wine <-> native: a Linux lantern (Alice) and a Windows lantern.exe under
...
#
#   WINEPREFIX=... scripts/wine-interop.sh <dir with lantern and lantern.exe>
set -euo pipefail
BIN="$(cd "${1:?directory holding lantern and lantern.exe}" && pwd)"
export WINEDEBUG=-all
DIR="$(mktemp -d)"
trap 'exec 3>&- 4>&- 2>/dev/null || true; kill $(jobs -p) 2>/dev/null || true; rm -rf "$DIR"' EXIT
mkfifo "$DIR/alice.in" "$DIR/bob.in"
"$BIN/lantern" --db "$DIR/alice.sqlite" --name Alice --relay off <"$DIR/alice.in" >"$DIR/alice.out" 2>&1 &
( cd "$DIR" && wine "$BIN/lantern.exe" --db bob.sqlite --name BobOnWine --relay off <"$DIR/bob.in" >"$DIR/bob.out" 2>&1 ) &
...
echo /invite >&4; wait_for bob "invite ltfriend"
echo "/accept $(grep -o 'ltfriend[a-z0-9]*' "$DIR/bob.out" | head -1)" >&3
...
echo "/channel invite Alice" >&4; wait_for alice "/channel join ltroom"
grep -o '/channel join ltroom[a-z0-9]*' "$DIR/alice.out" | head -1 >&3
...
for who in alice bob; do
  echo "===== $who"
  sed -E 's/(lt(friend|room|node)[a-z0-9]{12})[a-z0-9]+/\1.../g' "$DIR/$who.out" | grep -v XDG_RUNTIME_DIR
done
