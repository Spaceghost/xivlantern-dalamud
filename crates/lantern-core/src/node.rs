...
impl Node {
    ...
    /// Remember where a node can be reached (an `ltnode…` ticket), so dialing
    ...
    /// Publish an announcement signed offline (`ltannounce…`) into an author
...
mod tests {
    ...
    fn garbage_dial_targets_are_refused() {
        for s in ["", "nonsense", "ltroom", "ltnodezzz"] {
