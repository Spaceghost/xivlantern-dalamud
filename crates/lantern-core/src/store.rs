...
mod tests {
    ...
    fn identity_is_stable_across_reopen() {
        ...
        let path = dir.path().join("nested").join("lantern.sqlite");
    ...
    fn a_corrupt_identity_row_is_replaced_not_fatal() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("lantern.sqlite");
        {
    ...
    fn bookmarks_upsert_and_list_newest_first() {
        ...
        s.bookmark_put("a", Scope::Public, "Alpha", "ltroomaaa").unwrap();
        s.bookmark_put("b", Scope::Fc, "Beta", "ltroombbb").unwrap();
        s.bookmark_put("a", Scope::Public, "Alpha renamed", "ltroomaaa")
            .unwrap();
        ...
        let path = dir.path().join("v1.sqlite");
        {
            ...
            db.execute_batch(
                "CREATE TABLE identity (id INTEGER PRIMARY KEY CHECK (id = 1), secret BLOB NOT NULL, created_at INTEGER NOT NULL);
                 ...
                 INSERT INTO bookmarks VALUES ('kept', 4, 'Kept', 'ltroomx', 1);
                 PRAGMA user_version = 1;",
            )
            .unwrap();
        }
