...
impl Scope {
    ...
    /// and so on.
    ///
    /// Lantern carries metadata, not media. When a room later grows a video
...
pub const TICKET_PREFIX: &str = "ltroom";
...
pub const NODE_TICKET_PREFIX: &str = "ltnode";
...
pub const FRIEND_TICKET_PREFIX: &str = "ltfriend";
...
mod tests {
    ...
    fn friend_invites_round_trip_and_are_not_other_tickets() {
        ...
        // "ltfriend" does not start with "ltroom" or "ltnode", and vice versa.
        ...
        assert!(FriendInvite::from_str("ltfriendzzzz").is_err());
    }
    ...
    fn garbage_is_rejected_rather_than_panicking() {
        for s in ["", "ltroom", "ltroomzzzz", "not a ticket", "ltnode!!!!"] {
