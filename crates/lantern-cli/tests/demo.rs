//! The demo as a test: two `lantern` processes on one machine, relays off,
...
impl Cli {
    ...
    fn start_with(name: &'static str, dir: &std::path::Path, extra: &[&str]) -> Cli {
        let mut child = Command::new(env!("CARGO_BIN_EXE_lantern"))
...
fn two_clis_befriend_chat_and_share_a_channel() {
    ...
    alice.send("/invite");
    let invite = alice.expect("invite ltfriend");
    ...
    let join = bob.expect("/channel join ltroom");
    bob.send(&join);
...
fn author_tool(args: &[&str]) -> String {
    let out = Command::new(env!("CARGO_BIN_EXE_lantern"))
        .arg("author")
        .args(args)
        .output()
        .expect("run lantern author");
...
fn the_author_publishes_and_answers_a_player() {
    ...
    author.expect("ready ");
    let ticket = author.expect("ltnode").trim().to_string();
    ...
    let signed = author_tool(&["sign", "--key", key, "--seq", "1", "--title", "Hello", "--body", "Welcome to Lantern"]);
    ...
    player.expect("Welcome to Lantern");
