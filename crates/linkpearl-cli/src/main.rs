//! `linkpearl`: a line-oriented client for two people on a terminal, and the
//! demo for linkpearl-core. It drives the same synchronous `Node` handle the C
//! ABI wraps: commands are pushed, events are polled every few milliseconds.
//!
//! ```text
//! linkpearl --db alice.sqlite --name Alice --relay off
//! ```
//!
...
use linkpearl_core::{Config, Event, Node, PeerId, RelayMode, RoomHandle, Scope, Status};
...
commands:
  ...
  /accept <lpfriend...>       redeem somebody's invite
  ...
  /channel join <lproom...>   join a channel from a ticket
    ...
    fn event(&mut self, event: Event) {
        match event {
            ...
            Event::ChannelInvite { peer, ticket } => {
                let label = ticket
                    .parse::<linkpearl_core::RoomTicket>()
                    .map(|t| t.label)
                    .unwrap_or_default();
