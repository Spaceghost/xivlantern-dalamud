...
//!   else's NodeId.
//! * **invites over relays** — an `lpfriend` ticket sent as a NIP-17 private
//!   direct message (kind 14 rumor, sealed, gift-wrapped as kind 1059), marked
//!   with a `["linkpearl", "invite/1"]` tag on the rumor.
//!
...
pub const DEVICE_LIST_D: &str = "ffxiv-linkpearl/devices";
/// Marks a NIP-17 rumor as carrying a Linkpearl friend invite.
pub const INVITE_TAG: (&str, &str) = ("linkpearl", "invite/1");
const CLAIM_CONTEXT: &[u8] = b"linkpearl-device-claim/1";
...
pub fn parse_device_claims(event: &NostrEvent) -> Result<(PublicKey, Vec<DeviceClaim>)> {
    ...
    {
        return Err(Error::Arg("not a linkpearl device list"));
    }
    let content: DeviceListContent =
...
/// Open a gift wrap addressed to `me`. `Some` only for a Linkpearl invite
...
impl Relays {
    ...
    /// Every Linkpearl invite gift-wrapped to `me`.
...
mod tests {
    ...
    fn invites_travel_as_gift_wrapped_dms() {
        ...
        assert!(!wrap.content.contains("lpfriend"), "the ticket is encrypted");
