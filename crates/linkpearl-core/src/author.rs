...
//! announcement is signed offline with `linkpearl author sign` and pasted into
...
pub const ANNOUNCE_PREFIX: &str = "lpannounce";
const SIGN_CONTEXT: &[u8] = b"linkpearl-announcement/1";
...
mod tests {
    ...
    fn limits_are_enforced_on_both_ends() {
        ...
        assert!(SignedAnnouncement::from_str("lpannouncezz").is_err());
        assert!(SignedAnnouncement::from_str("lpfriendabc").is_err());
    }
