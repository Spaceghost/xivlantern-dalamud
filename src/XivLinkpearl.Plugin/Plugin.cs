...
/// <summary>
/// /linkpearl: a friend list, 1:1 chat and channels over iroh, peer to peer, plus an opt-in channel for the mod's
...
public sealed class Plugin : IDalamudPlugin
{
    ...
    private readonly WindowSystem windowSystem = new("XivLinkpearl");
    private readonly Configuration config = null!;
    private readonly LinkpearlService service = null!;
        ...
        INotificationManager notifications, IDtrBar dtrBar)
    {
        ...
        try
        {
            ...
            // Dalamud loads the managed assembly from memory; linkpearl.dll is found by the folder it was installed to.
            ...
            service = new LinkpearlService(nativeDir, pluginInterface.ConfigDirectory.FullName, config, Save, log, framework);
    ...
    private void Print(string line) => chat.Print("[Linkpearl] " + line);
    ...
    private void DrawUi()
    {
        ...
        {
            log.Error(ex, "Linkpearl UI draw failed");
        }
    }
        ...
        try
        {
            ...
            switch (c.Verb)
            {
                ...
                case Verb.Off:
                    ...
                    break;
                case Verb.SelfTest:
                    Print(service.IsOn ? "testing this node (up to a minute)…" : "Linkpearl is off: testing with a temporary node that is closed afterwards (up to a minute)…");
