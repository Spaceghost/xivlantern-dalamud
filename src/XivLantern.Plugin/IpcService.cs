...
public sealed class IpcService : IDisposable
{
    ...
    public IpcService(IDalamudPluginInterface pi, LanternService service, Configuration config, Action<string> openChat)
    {
    ...
    private string Friends()
    {
        if (!config.AllowIpcRead)
            return "error: the player has not allowed other plugins to read Lantern";
        return service.IsOn ? IpcPayloads.Friends(service.Friends) : "error: Lantern is off";
    }

    private string Send(string who, string text)
    {
        if (!config.AllowIpcSend)
            return "error: the player has not allowed other plugins to send on Lantern";
        if (!service.IsOn)
            return "error: Lantern is off";
