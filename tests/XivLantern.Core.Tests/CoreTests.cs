...
using XivLantern.Shared;

namespace XivLantern.Core.Tests;
...
public class CommandTests
{
    ...
    [Fact]
    public void TheCommandAndItsAlias()
    {
        Assert.Equal("/lantern", Command.Name);
        Assert.Equal("/xivlantern", Command.Alias);
        Assert.Contains(Command.Name, Command.Help);
    }
    ...
    public void MessagesKeepTheirSpacing()
    {
        ...
        Assert.Equal("ltfriendabc", Command.Parse("accept ltfriendabc").Arg);
    }
...
public class TextAndTicketTests
{
    ...
    {
        var (kind, t) = Tickets.Find("Alice: add me! \"ltfriendafwxmtvu3li6abcdefgh\".");
        Assert.Equal(TicketKind.FriendInvite, kind);
        Assert.Equal("ltfriendafwxmtvu3li6abcdefgh", t);
        Assert.Equal(TicketKind.Room, Tickets.Find("LTROOMAFYKIA7HGVIUABCD").Kind);
        Assert.Equal(TicketKind.Announcement, Tickets.Find("ltannounceabcdefghijk").Kind);
        Assert.Equal(TicketKind.None, Tickets.Find("ltfriend").Kind);
        Assert.Equal(TicketKind.None, Tickets.Find("ltfriend0189!!!!!!!!!!").Kind);
...
public class IpcAndIdentityTests
{
    ...
    public void IpcNamesAreVersioned()
    {
        foreach (var f in typeof(IpcContract).GetFields(BindingFlags.Public | BindingFlags.Static))
            Assert.StartsWith("XivLantern.v1.", (string)f.GetValue(null)!);
    }
...
public class PrivacyTests
{
    ...
    {
        var dir = Path.Combine(RepoRoot, "src", "XivLantern.Plugin");
