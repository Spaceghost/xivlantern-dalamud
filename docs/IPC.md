...
> the payloads are unit tested (`tests/XivLinkpearl.Core.Tests`). No other
> plugin has called these gates yet, and none of it has run in game.

XivLinkpearl publishes Dalamud IPC gates so sibling plugins (Ghostty, XivMcp,
a launcher) can use the player's Linkpearl without referencing its assembly.
...
matching box in Linkpearl's settings ("Let other plugins see my friends and
...
| Gate | Shape | Needs |
| --- | --- | --- |
| `Linkpearl.v1.GetStatus` | `Func<string>` | nothing |
| `Linkpearl.v1.GetFriends` | `Func<string>` | read allowed |
| `Linkpearl.v1.SendToFriend` | `Func<string who, string text, string>` | send allowed |
| `Linkpearl.v1.OpenChat` | `Action<string who>` | nothing |
| `Linkpearl.v1.MessageReceived` | event, `string` | read allowed |
...
(`error: the player has not allowed other plugins to send on Linkpearl`,
`error: Linkpearl is off`, `error: no friend called "x"`, …). The message is
...
Opens the Linkpearl window, on that friend's chat when `who` names one, else
...
```csharp
var friends = pi.GetIpcSubscriber<string>("Linkpearl.v1.GetFriends").InvokeFunc();
var result  = pi.GetIpcSubscriber<string, string, string>("Linkpearl.v1.SendToFriend").InvokeFunc("Bob", "hi");
pi.GetIpcSubscriber<string, object>("Linkpearl.v1.MessageReceived").Subscribe(json => { /* ... */ });
```

A caller must expect `IpcNotReadyError` when XivLinkpearl is not installed or
