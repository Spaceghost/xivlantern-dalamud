#!/usr/bin/env bash
# Package a built XivLantern plugin for Dalamud, and write the one-entry plugin
...
#
#   dotnet build XivLantern.slnx -c Release && tools/package.sh
#
# Output, all under $OUT (default ../xiv-lantern-build/release):
#   latest.zip                  the plugin folder as Dalamud installs it (XivLantern.json,
#                               XivLantern.dll, XivLantern.Core.dll, Lantern.Interop.dll and
#                               the native lantern.dll, no .pdb). The name is stable on purpose, so
#                               .../releases/latest/download/latest.zip never changes.
#   XivLantern-<version>.zip       the same bytes under a self-describing name
...
#                      $XIVLANTERN_ARTIFACTS/bin/XivLantern.Plugin/release, else
#                      ../xiv-lantern-build/artifacts/bin/XivLantern.Plugin/release)
#   OUT                where to write (default ../xiv-lantern-build/release)
#   REPO               owner/name on GitHub (default $GITHUB_REPOSITORY, else
#                      Spaceghost/xivlantern-dalamud)
...
ARTIFACTS="${XIVLANTERN_ARTIFACTS:-$ROOT/../xiv-lantern-build/artifacts}"
BIN="${BIN:-$ARTIFACTS/bin/XivLantern.Plugin/release}"
OUT="${OUT:-$ROOT/../xiv-lantern-build/release}"
REPO="${REPO:-${GITHUB_REPOSITORY:-Spaceghost/xivlantern-dalamud}}"
MANIFEST="$ROOT/src/XivLantern.Plugin/XivLantern.json"

[[ -f "$BIN/XivLantern.dll" ]] || {
  echo "error: $BIN/XivLantern.dll missing; run: dotnet build XivLantern.slnx -c Release" >&2
  exit 1
}
...
STAGE="${TMPDIR:-/tmp}/xivlantern-package-$$"
trap 'rm -rf "$STAGE"' EXIT
rm -rf "$STAGE"
mkdir -p "$STAGE/XivLantern" "$OUT"
...
( cd "$BIN" && find . -type f -print0 | while IFS= read -r -d '' f; do
    ...
    case "$f" in
      ...
      ./runtimes/* | ./XivLantern/*) continue ;;                          # other platforms; DalamudPackager's own staging copy
      ./*.deps.json | ./*.runtimeconfig.json | *.pdb | *.zip) continue ;;
      ./*/*) continue ;;                                               # nothing else from a subfolder
      ./*.dll | ./*.json) ;;
      *) continue ;;
    esac
    mkdir -p "$STAGE/XivLantern/$(dirname "$f")"
    cp "$f" "$STAGE/XivLantern/$f"
  done )
cp "$MANIFEST" "$STAGE/XivLantern/XivLantern.json"

# Dalamud unpacks the zip straight into the plugin folder, so the files sit at the root.
find "$STAGE/XivLantern" -exec touch -h -d "@$SOURCE_DATE_EPOCH" {} +
rm -f "$OUT/latest.zip" "$OUT/XivLantern-$VERSION.zip"
( cd "$STAGE/XivLantern" && find . -type f | LC_ALL=C sort | sed 's|^\./||' |
    TZ=UTC zip -X -D -q "$OUT/latest.zip" -@ )
cp "$OUT/latest.zip" "$OUT/XivLantern-$VERSION.zip"
echo "== $OUT/latest.zip"
...
# lantern.dll is the native library: without it the plugin loads and cannot connect.
for want in XivLantern.dll XivLantern.json XivLantern.Core.dll Lantern.Interop.dll lantern.dll ; do
